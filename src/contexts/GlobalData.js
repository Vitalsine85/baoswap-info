...
async function getGlobalData(
  ...
  try {
    ...
    data = result.data.baoswapFactories[0];
    ...
    oneDayData = oneDayResult.data.baoswapFactories[0];

    let twoDayResult = await client.query({
      query: GLOBAL_DATA(factoryAddress, twoDayBlock?.number),
      fetchPolicy: "network-only",
    });
    twoDayData = twoDayResult.data.baoswapFactories[0];

    let oneWeekResult = await client.query({
      query: GLOBAL_DATA(factoryAddress, oneWeekBlock?.number),
      fetchPolicy: "network-only",
    });
    const oneWeekData = oneWeekResult.data.baoswapFactories[0];

    let twoWeekResult = await client.query({
      query: GLOBAL_DATA(factoryAddress, twoWeekBlock?.number),
      fetchPolicy: "network-only",
    });
    const twoWeekData = twoWeekResult.data.baoswapFactories[0];
...
const getChartData = async (client, oldestDateToFetch) => {
  ...
  try {
    while (!allFound) {
      ...
      skip += 1000;
      data = data.concat(result.data.baoswapDayDatas);
      if (result.data.baoswapDayDatas.length < 1000) {
        allFound = true;
      }
    }
